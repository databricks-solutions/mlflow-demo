...
class SetupValidator:
    ...
    def validate_prerequisites(self) -> Tuple[bool, List[str]]:
        ...
        self.validation_results['prerequisites'] = {
            'valid': all_valid,
            'issues': issues
        }
    ...
    def validate_environment_config(self, config: Dict[str, str]) -> Tuple[bool, List[str]]:
        ...
        all_valid = len(issues) == 0
        self.validation_results['environment_config'] = {
            'valid': all_valid,
            'issues': issues
        }
    ...
    def validate_resource_creation(self, resources: Dict[str, Any]) -> Tuple[bool, List[str]]:
        ...
        if 'catalog_name' in resources and 'schema_name' in resources:
            if not self._validate_schema_access(resources['catalog_name'], resources['schema_name']):
                issues.append(f"Schema {resources['catalog_name']}.{resources['schema_name']} not accessible")
        ...
        all_valid = len(issues) == 0
        self.validation_results['resource_creation'] = {
            'valid': all_valid,
            'issues': issues
        }
    ...
    def validate_deployment(self, app_name: str, app_url: Optional[str] = None) -> Tuple[bool, List[str]]:
        ...
        all_valid = len(issues) == 0
        self.validation_results['deployment'] = {
            'valid': all_valid,
            'issues': issues
        }
    ...
    def generate_validation_report(self) -> str:
        ...
        report.append(f"Overall Status: {'✅ ALL CHECKS PASSED' if overall_status else '❌ ISSUES FOUND'}")
